...
var Tree = function Tree(){
  var node = new Node({item:'project',isOpen:true});
  this._root = node;
}
...
Tree.prototype.traverseBFOrder = function(callback){
  ...
  while(currentTree){
    ...
    var childrenPriceData =[];
    if(currentTree.children.length>0){
      childrenPriceData = currentTree.children.map(function(obj,index){
        return Number(obj.data.contractPrice);
      })
      console.log(currentTree.data.code,":",childrenPriceData)
      currentTree.data.contractPrice= _.sum(childrenPriceData);
    }
...
Tree.prototype.add = function(data,toData,traversal,item){
  var child = new Node(data);
  var parent = null;
  var callback = function(node){
    if(node.data[item] === toData){
      parent = node;
    }
  }
...
function findIndex(arr,data,item){
  var index;
  for(var i=0;i<arr.length;i++){
    if(arr[i].data[item]==data){
      index = i;
    }
  }
  return index;
